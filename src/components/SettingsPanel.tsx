...
import { Plus, Trash2, GripVertical, Check, X, User } from 'lucide-react';
    ...
    <div className="space-y-2">
      ...
      <div className="flex items-center gap-2 pt-1">
        ...
        <input
          ...
          className="w-24 border border-gray-200 rounded-lg px-2 py-1 text-xs font-mono focus:outline-none focus:ring-2 focus:ring-indigo-300"
          placeholder="#000000"
          spellCheck={false}
        />
...
function EditableRow({ label, color, onUpdate, onDelete, renderPreview }: {
  ...
  if (editing) {
    return (
      <div className="rounded-xl border border-indigo-200 bg-indigo-50/40 p-3 space-y-2">
        <input autoFocus className="w-full border border-gray-200 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300"
        ...
        <div className="flex gap-2">
          ...
          <button onClick={cancel} className="px-3 py-1.5 text-gray-500 text-xs rounded-lg hover:bg-gray-100">Cancel</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex items-center gap-2 px-3 py-2 rounded-xl border border-gray-100 bg-white hover:border-gray-200 group">
      <GripVertical size={14} className="text-gray-300 cursor-grab shrink-0" />
      {renderPreview(label, color)}
      <div className="flex-1" />
      <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition">
        <button onClick={() => setEditing(true)} className="p-1 rounded hover:bg-gray-100 text-gray-400 text-xs">Edit</button>
        <button onClick={onDelete} className="p-1 rounded hover:bg-red-50 text-red-400"><Trash2 size={12} /></button>
      </div>
    </div>
  );
}
  ...
  if (editing) {
    return (
      <div className="rounded-xl border border-indigo-200 bg-indigo-50/40 p-3 space-y-2">
        ...
        <input type="email" className="w-full border border-gray-200 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300"
  ...
  return (
    <div className="rounded-xl border border-indigo-200 bg-indigo-50/40 p-3 space-y-2">
      ...
      <div className="flex gap-2">
        ...
        <button onClick={onCancel} className="px-3 py-1.5 text-gray-500 text-xs rounded-lg hover:bg-gray-100">Cancel</button>
      </div>
    </div>
  );
}
  ...
  return (
    <section>
      <div className="flex items-center justify-between mb-3">
        ...
        <button onClick={onAdd} className="flex items-center gap-1 text-xs text-indigo-600 hover:underline">
          <Plus size={12} /> Add
        </button>
      </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xl mx-4 overflow-hidden" onClick={e => e.stopPropagation()}>
        ...
        <div className="overflow-y-auto max-h-[75vh] px-6 py-5 space-y-8">
          ...
          {/* Google Calendar */}
          <section>
            <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">Google Calendar</h3>
            <div className="space-y-2">
              <input
          ...
          <Section title="People & Delegates" onAdd={() => setAdding('person')}>
            ...
            {settings.people.length === 0 && adding !== 'person' && (
              <p className="text-xs text-gray-400 italic">No people yet — add teammates to delegate tasks to them.</p>
            )}
            {adding === 'person' && (
              <AddForm
